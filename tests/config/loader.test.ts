...
describe('ConfigLoader', () => {
  ...
  it('loads and merges configuration files', async () => {
    ...
    vi.mocked(fs.readdir).mockResolvedValue(['config1.json', 'config2.json', 'ignored.txt'] as any);
    vi.mocked(fs.readFile).mockImplementation(async (path: any) => {
      if (String(path).endsWith('config1.json')) return JSON.stringify(file1);
      if (String(path).endsWith('config2.json')) return JSON.stringify(file2);
      return '';
    });
  ...
  it('filters servers by tag (ANY match)', async () => {
    ...
    vi.mocked(fs.readdir).mockResolvedValue(['f1.json', 'f2.json'] as any);
    vi.mocked(fs.readFile).mockImplementation(async (path: any) => {
        if (String(path).endsWith('f1.json')) return JSON.stringify(file1);
  ...
  it('ignores invalid JSON files', async () => {
      vi.mocked(fs.readdir).mockResolvedValue(['bad.json'] as any);
